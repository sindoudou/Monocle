...
abstract class Getter[S, A] extends Serializable { self =>
  ...
  final def sum[S1](other: Getter[S1, A]): Getter[S Xor S1, A] =
    Getter[S Xor S1, A](_.fold(self.get, other.get))

  /** pair two disjoint [[Getter]] */
  final def product[S1, A1](other: Getter[S1, A1]): Getter[(S, S1), (A, A1)] =
...
sealed abstract class GetterInstances0 {
  implicit val getterChoice: Choice[Getter] = new Choice[Getter]{
    override def choice[A, B, C](f: Getter[A, C], g: Getter[B, C]): Getter[A Xor B, C] =
      f sum g

    override def id[A]: Getter[A, A] =
      Getter.id

    override def compose[A, B, C](f: Getter[B, C], g: Getter[A, B]): Getter[A, C] =
      g composeGetter f
  }
}